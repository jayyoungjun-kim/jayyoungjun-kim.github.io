...
export function structure(source,page,fields=[]) {
 ...
 if(root&&!['index.html','about.html','info.html'].includes(page)){
  ...
  for(const n of children(root)){
   ...
   if(fixed(n)){
    flush();
    if(has(n,'project-stakeholder-container')){
     const group=add(n,'participants',null,false);
     const grid=find(n,x=>has(x,'project-stakeholder-grid'));
     for(const column of children(grid||n)){
      const parts=children(column);
      for(let i=0;i<parts.length;i++)if(has(parts[i],'project-stakeholder-title')){
       const entry=add(parts[i],'participant',group.id);
       if(has(parts[i+1]||{},'project-stakeholder-body'))entry.end=loc(parts[++i]).endOffset;
      }
     }
    }else if(has(n,'project-pagination-container')){
     const group=add(n,'navigation',null,false);group.directions=[];
     const grid=find(n,x=>has(x,'project-pagination-grid'));
     for(const entry of children(grid||n))if(has(entry,'project-pagination')){
      const direction=find(entry,x=>has(x,'project-pagination-next'))?'next':'previous';
      const child=add(entry,'projectLink',group.id);child.direction=direction;child.title=direction==='next'?'다음 프로젝트':'이전 프로젝트';group.directions.push(direction);
     }
    }else add(n,'fixed',null,false);
    continue;
   }
 ...
 return {sections:items,rootKinds:page==='index.html'?['category']:page==='about.html'?['aboutGroup']:page==='info.html'?['infoGroup']:root?['story','gallery','text',...(!items.some(s=>s.kind==='participants')?['participants']:[]),...(!items.some(s=>s.kind==='navigation')?['navigation']:[])]:[],_nodes:nodes,_root:root,_doc:doc};
}
export const templateNames={participants:'프로젝트 참여자',participant:'참여자',navigation:'이전 · 다음 프로젝트',previous:'이전 프로젝트',next:'다음 프로젝트',story:'이미지 + 텍스트',gallery:'이미지 모음',category:'작업 카테고리',card:'작업 카드',aboutGroup:'소개 섹션',infoGroup:'정보 섹션',aboutItem:'제목 + 한국어·영어 본문',infoItem:'한국어·영어 본문',links:'링크 목록',news:'날짜 + 소식',text:'제목 + 본문',image:'이미지',video:'영상',twoImages:'이미지 2열',spacer:'여백',paragraph:'본문 문단',list:'목록',link:'링크',footnote:'각주'};
export function allowedTemplates(s,page){if(!s)return structure('',page).rootKinds;
 if(s.kind==='participants')return ['participant'];
 if(s.kind==='navigation')return ['next','previous'].filter(d=>!s.directions.includes(d));
...
function template(type,page,title=''){
 ...
 switch(type){
 case'participants':return '<div class="project-stakeholder-container"><div class="project-stakeholder-grid"><div class="project-stakeholder"></div><div class="project-stakeholder"></div></div></div>';
 case'participant':return '<div class="project-stakeholder-title">역할을 입력하세요</div><div class="project-stakeholder-body">'+t+'</div>';
 case'navigation':return '<div class="project-pagination-container"><div class="project-pagination-grid"></div></div>';
 case'previous':case'next':return '<div class="project-pagination"><div class="project-pagination-'+type+'"><a href="index.html">'+(type==='next'?'← Next Project':'Previous Project →')+'</a></div><div class="project-pagination-'+type+'-project">'+t+'</div></div>';
...
export function changeStructure(source,page,op){
 ...
 if(op.action==='add'){
  ...
  if(parent?.kind==='navigation'){
   const grid=find(container,n=>has(n,'project-pagination-grid'));
   if(!grid){
    const start=loc(container).startTag.endOffset,end=loc(container).endTag.startOffset;
    const wrapped=source.slice(0,start)+'<div class="project-pagination-grid">'+source.slice(start,end)+'</div>'+source.slice(end);
    return changeStructure(wrapped,page,op);
   }
   container=grid;
  }
  if(parent?.kind==='participants'){
   const grid=find(container,n=>has(n,'project-stakeholder-grid'))||container;
   const columns=children(grid).filter(n=>has(n,'project-stakeholder'));
   container=columns.sort((a,b)=>children(a).length-children(b).length)[0];
  }
  ...
  const nextLink=parent?.kind==='navigation'&&op.template==='next'?children(container)[0]:null;
  const at=nextLink?loc(nextLink).startOffset:target?(op.position==='before'?target.start:target.end):firstText?loc(firstText).startOffset:footer?loc(footer).startOffset:loc(container).endTag.startOffset;
  let addition=template(op.template,page,op.title);
  if(!parent&&page!=='index.html'&&!['about.html','info.html'].includes(page)&&!['story','gallery','participants','navigation'].includes(op.template))addition='<section data-cms-section="true">'+addition+'</section>';
  ...
  else if(op.action==='duplicate'){requireValue(s.kind!=='projectLink',422,'이전·다음 프로젝트는 각각 하나만 연결할 수 있습니다.');result=source.slice(0,s.end)+'\n'+snippet+source.slice(s.end);}
...
function normalize(source,page){
 ...
 groups.forEach((s,i)=>{const n=m._nodes.get(s.id),a=loc(n).attrs.class;patches.push({start:a.startOffset,end:a.endOffset,text:`class="${family}-container${i===groups.length-1?'-last':''}"`});
  ...
  news.forEach((item,index)=>{const date=children(item).find(x=>has(x,'info-section-date')||has(x,'info-section-date-first'));if(!date)return;const a=loc(date).attrs.class;const classes=cls(date).map(c=>c==='info-section-date'||c==='info-section-date-first'?'info-section-date'+(index===0?'-first':''):c);patches.push({start:a.startOffset,end:a.endOffset,text:`class="${classes.join(' ')}"`});});
