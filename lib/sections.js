...
export function structure(source,page,fields=[]) {
 ...
 function add(n,kind,parent=null,movable=true){if(!loc(n))return;
  ...
  const s={id:id(n),kind,parent,title:(preferred?text(preferred):text(n)).trim().replace(/\s+/g,' ').slice(0,72)|| (image?'이미지':kinds[kind]),thumbnail:image?attr(image,'src')||'':'',movable,start:loc(n).startOffset,end:loc(n).endOffset,children:[],fieldIds:[]};
  if(kind==='detail'&&!preferred)s.title=image?'이미지':find(n,x=>['video','iframe'].includes(x.tagName))?'영상':s.title;
  if(kind==='detail'&&(has(n,'project-body-empty')||has(n,'project-body-between')))s.title='여백';
  if(kind==='detail'&&has(n,'project-stakeholder-container'))s.title='프로젝트 참여자';
  if(kind==='detail'&&has(n,'project-pagination-container'))s.title='이전 · 다음 프로젝트';
 ...
 return {sections:items,rootKinds:page==='index.html'?['category']:page==='about.html'?['aboutGroup']:page==='info.html'?['infoGroup']:root?['text','image','video','twoImages','spacer']:[],_nodes:nodes,_root:root,_doc:doc};
}
export const templateNames={category:'작업 카테고리',card:'작업 카드',aboutGroup:'소개 섹션',infoGroup:'정보 섹션',aboutItem:'제목 + 한국어·영어 본문',infoItem:'한국어·영어 본문',links:'링크 목록',news:'날짜 + 소식',text:'제목 + 본문',image:'이미지',video:'영상',twoImages:'이미지 2열',spacer:'여백',paragraph:'본문 문단',list:'목록',link:'링크',footnote:'각주'};
...
export function changeStructure(source,page,op){
 ...
 if(op.action==='add'){
  ...
  requireValue((parent?allowedTemplates(parent,page):model.rootKinds).includes(op.template),422,'이 위치에 추가할 수 없는 유형입니다.');
  ...
  const at=footer?loc(footer).startOffset:loc(container).endTag.startOffset;
  result=source.slice(0,at)+'\n'+template(op.template,page,op.title)+'\n'+source.slice(at);
 }else{
  requireValue(s&&s.movable,422,'이 섹션은 구조를 변경할 수 없습니다.');
  const node=model._nodes.get(s.id),snippet=source.slice(s.start,s.end);
