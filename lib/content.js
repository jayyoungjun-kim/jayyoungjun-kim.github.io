...
export function inspect(source, page) {
  ...
  walk(doc,(n,parents)=>{if(!n.sourceCodeLocation?.endTag||parents.some(p=>richNodes.includes(p)))return;
    if(classes(n).some(c=>/^(about-section-(title|body.*)|info-section-(body.*|public-title|public-body.*|date)|project-body-(title|kr|en|label)|project-header-body-(kr|en))$/.test(c)))richNodes.push(n);
  });
  let title = page;
  walk(doc, (n, parents) => {
    ...
    if(richNodes.includes(n)){
      ...
      fields.push({id:`r${loc.startOffset}`,type:'text',label:label(n,parents),value,plain:plainRich(value),rich:true,start:loc.startTag.endOffset,end:loc.endTag.startOffset,kind:'rich'});
      return;
    }
...
export function applyEdits(source, page, changes) {
  ...
  for (const [id, value] of Object.entries(changes)) {
    ...
    let replacement;
    if(f.kind==='rich')replacement=cleanRich(value)||'<!--cms-empty-->';
    else if (f.kind === 'text') {
