...
import { changeStructure } from './sections.js';
...
export class LocalAdmin {
  ...
  data(row, remoteSha) {
    ...
    return {...data,source:row.source,page:row.page,fields:fields.map(({start,end,kind,...f})=>f),revision:row.revision,
      undoAvailable:!!row.undo?.length,draftId:row.draftId||null,baseSha:row.baseSha,updatedAt:row.updatedAt||null,
  ...
  async request(route,{method='GET',body:input={}}={}) {
    ...
    if(path==='/api/deployment') {
      ...
      const live=await github.request('/live-status');
      if(live.revision===sha||live.published?.includes(sha))return{status:'completed',conclusion:'success',live:true};
    ...
    if(path==='/api/media'&&method==='GET'){
      const remote=await github.media(),local=await this.store.all('uploads');
      const files=new Map(remote.files.map(f=>[f.path.split('/').pop(),{name:f.path.split('/').pop(),url:'/'+f.path,size:f.size,label:remote.catalog?.[f.path.split('/').pop()]?.label,published:true}]));
      for(const f of local)if(/\.(png|jpg|gif|webp|mp4)$/.test(f.name))files.set(f.name,{...files.get(f.name),name:f.name,label:f.label,url:'/assets/uploads/'+f.name,size:f.blob.size,published:files.has(f.name)});
      return {files:[...files.values()]};
    }
    if(path==='/api/media'&&['POST','DELETE'].includes(method)){
      const name=input.name;
      requireValue(typeof name==='string'&&/^[a-f0-9-]{36}\.(png|jpg|gif|webp|mp4)$/.test(name),422,'이미지 또는 영상을 선택해 주세요.');
      const file=await this.store.get('uploads',name);
      if(method==='POST'){
        requireValue(file,404,'업로드할 임시 파일이 없습니다.');
        return github.publishAsset(name,new Uint8Array(await file.blob.arrayBuffer()),undefined,file.label);
      }
      const remote=await github.pages(),assetPath='/assets/uploads/'+name;
      const drafts=await this.store.all('drafts');
      requireValue(!drafts.some(d=>d.source.includes(assetPath)),409,'저장된 초안에서 사용 중인 파일입니다. 먼저 파일 연결을 변경해 주세요.');
      for(const page of remote.pages){
        const original=await github.file(page.path,remote.head.sha);
        requireValue(!original.source.includes(assetPath),409,page.path+'에서 사용 중인 파일입니다. 먼저 파일 연결을 변경하고 발행해 주세요.');
      }
      const media=await github.media(remote.head);let result={ok:true};
      if(media.files.some(f=>f.path==='assets/uploads/'+name))result=await github.publishAsset(name,null,remote.head);
      if(file)await this.store.mutate('uploads',name,()=>null);
      return result;
    }
    if(path==='/api/uploads'&&method==='POST') {
      ...
      await this.store.mutate('uploads',name,()=>({name,blob:input,label:typeof input.name==='string'?input.name:name}));
    ...
    if(['/api/draft','/api/preview','/api/block','/api/backup','/api/structure','/api/undo'].includes(path)&&method==='POST') {
      ...
      if(path==='/api/structure') {
        const before=inspect(row.source,page).sections,after=inspect(source,page).sections;
        const operation={...input.operation};
        for(const key of ['id','parent','target'])if(operation[key]){const index=before.findIndex(s=>s.id===operation[key]);requireValue(index>=0,422,'섹션을 다시 선택해 주세요.');operation[key]=after[index].id;}
        row.undo=[...(row.undo||[]),source].slice(-10);
        source=changeStructure(source,page,operation);
      }
      if(path==='/api/undo') {
        requireValue(!Object.keys(input.changes||{}).length,409,'먼저 현재 문구를 저장한 뒤 구성을 되돌려 주세요.');
        requireValue(row.undo?.length,422,'되돌릴 구성 변경이 없습니다.');
        row.undo=[...row.undo];source=row.undo.pop();
      }
      if(path==='/api/block') {
    ...
    if(path==='/api/publish'&&method==='POST') {
      ...
      const names=uploadNames(row.source);
      const published=new Set(names.some(n=>/\.(png|jpg|gif|webp|mp4)$/.test(n))?(await github.media()).files.map(f=>f.path.split('/').pop()):[]);
      for(const name of names) {
        if(published.has(name))continue;
        const file=await this.store.get('uploads',name);
        if(file){
          size+=file.blob.size;requireValue(size<=24*1024*1024,413,'한 번에 발행하는 파일은 합계 24MB까지입니다.');
          assets.push({label:/\.(png|jpg|gif|webp|mp4)$/.test(name)?file.label:undefined,path:`assets/uploads/${name}`,bytes:new Uint8Array(await file.blob.arrayBuffer())});
        }else{
