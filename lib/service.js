...
export class LocalAdmin {
  ...
  async request(route,{method='GET',body:input={}}={}) {
    ...
    if(path==='/api/media'&&method==='GET'){
      ...
      for(const f of local)if(/\.(png|jpg|gif|webp)$/.test(f.name))files.set(f.name,{...files.get(f.name),name:f.name,label:f.label,url:'/assets/uploads/'+f.name,size:f.blob.size,published:files.has(f.name)});
      return {files:[...files.values()]};
    }
    if(path==='/api/media'&&['POST','DELETE'].includes(method)){
      const name=input.name;
      requireValue(typeof name==='string'&&/^[a-f0-9-]{36}\.(png|jpg|gif|webp)$/.test(name),422,'이미지를 선택해 주세요.');
      const file=await this.store.get('uploads',name);
      if(method==='POST'){
        requireValue(file,404,'업로드할 임시 이미지가 없습니다.');
      ...
      requireValue(!drafts.some(d=>d.source.includes(assetPath)),409,'저장된 초안에서 사용 중인 이미지입니다. 먼저 이미지 연결을 변경해 주세요.');
      for(const page of remote.pages){
        const original=await github.file(page.path,remote.head.sha);
        requireValue(!original.source.includes(assetPath),409,page.path+'에서 사용 중인 이미지입니다. 먼저 이미지 연결을 변경하고 발행해 주세요.');
      }
    ...
    if(path==='/api/publish'&&method==='POST') {
      ...
      const published=new Set(names.some(n=>/\.(png|jpg|gif|webp)$/.test(n))?(await github.media()).files.map(f=>f.path.split('/').pop()):[]);
      for(const name of names) {
        if(published.has(name))continue;
        const file=await this.store.get('uploads',name);
        if(file){
          size+=file.blob.size;requireValue(size<=24*1024*1024,413,'한 번에 발행하는 파일은 합계 24MB까지입니다.');
          assets.push({label:/\.(png|jpg|gif|webp)$/.test(name)?file.label:undefined,path:`assets/uploads/${name}`,bytes:new Uint8Array(await file.blob.arrayBuffer())});
        }else{
