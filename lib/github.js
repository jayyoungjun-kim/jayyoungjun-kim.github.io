...
export class GitHub {
  ...
  async media(ref) {
    ...
    return {head,catalog,files:tree.tree.filter(f=>f.type==='blob'&&/^assets\/uploads\/[a-f0-9-]+\.(png|jpg|gif|webp)$/.test(f.path))};
  }
  async publishAsset(name,bytes,head,label) {
    requireValue(/^[a-f0-9-]{36}\.(png|jpg|gif|webp)$/.test(name),422,'이미지 파일을 확인해 주세요.');
