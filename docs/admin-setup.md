...
- ABOUT / INFO: 큰 섹션과 그 안의 내용 블록을 별도로 추가·복제·삭제한다. 각 항목은 제목·국문·영문·캡션으로 편집한다. 본문은 중간 각주 번호나 HTML 줄바꿈 때문에 쪼개지지 않는다. Enter로 줄을 바꾸고 굵게·목록·링크 도구를 사용할 수 있다. 기존 본문 각주 표시는 편집칸에서 제외되고 해당 문단을 수정·발행하면 제거된다. 캡션의 [1] 등 출처 표기는 그대로 편집할 수 있다.
- 상세 페이지: 이미지 + 텍스트, 이미지 모음 또는 텍스트 섹션을 추가한다. 한 섹션 안에 이미지·영상·설명을 여러 개 넣고 각각 순서를 바꿀 수 있다. 기존 인접 이미지와 설명은 함께 묶고 여백도 함께 이동하므로 여백 항목을 따로 관리하지 않는다. 새 섹션에는 여백이 포함된다. 프로젝트 기본 정보와 이전/다음 탐색은 내용만 수정한다.
